...
ci::Vec2f tol::Transform::skewCorrection(const ci::Vec2f & vec_, const float & max_value_)
{
    Vec2f v;
    v.x = vec_.x / max_value_;
    v.y = vec_.y / max_value_;

    if (v.x * v.x + v.y * v.y > 0.000001f)
    {
        // â‘Î’l‚Ì¬‚³‚¢•û‚Å‘å‚«‚¢•û‚ğŠ„‚é
        float a = (fabs(v.x) > fabs(v.y)) ? v.y / v.x : v.x / v.y;
        // •â³”ä
        float c = 1.0f / sqrt(1.0f + a * a);
        v *= Vec2f(c, c);
    }
    return v;
}
